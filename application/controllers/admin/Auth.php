...
class Auth extends MY_Controller {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if($result){
					...
					if($result['is_admin'] == 1){
						...
						$_SESSION['ck_username'] = $result['username'];
						if($result['is_supper'])
	...
	public function register(){
			...
			if($this->input->post('submit')){
				...
				$this->form_validation->set_rules('username', 'Username', 'trim|alpha_numeric|is_unique[ci_admin.username]|required|xss_clean|strip_tags');
				$this->form_validation->set_rules('firstname', 'Firstname', 'trim|required|xss_clean|strip_tags');
				$this->form_validation->set_rules('lastname', 'Lastname', 'trim|required|xss_clean|strip_tags');
				$this->form_validation->set_rules('email', 'Email', 'trim|valid_email|is_unique[ci_admin.email]|required|xss_clean|strip_tags');
				$this->form_validation->set_rules('password', 'Password', 'trim|required|min_length[8]|xss_clean|strip_tags');
				$this->form_validation->set_rules('confirm_password', 'Password Confirmation', 'trim|required|matches[password]|xss_clean|strip_tags');
